...
const PostsPage = () => {
  ...
  const type = 'post';
  const pluralType = 'posts';
  const capitalType = 'Post';

  // Fetch posts from backend (MongoDB) once auth state is known
  useEffect(() => {
  ...
  const handleSubmit = async (payload) => {
    ...
    try {
      if (editingItem) {
        await api.put(`/api/posts/${editingItem._id}`, payload);
      } else {
